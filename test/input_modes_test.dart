...
void main() {
  testWidgets(
    'con el lápiz activo el dedo no dibuja: selecciona y mueve elementos',
    (tester) async {
      ...
      await _drag(
        tester,
        [for (var i = 0; i <= 5; i++) Offset(1000.0 - i * 20, 700)],
      ...
      await _drag(
        tester,
        [for (var i = 0; i <= 6; i++) before.center + Offset(0, 15.0 * i)],
        PointerDeviceKind.touch,
        pointer: 3,
      );
      expect(
        tester.getRect(find.text(_node1)).top - before.top,
        closeTo(90, 0.5),
      );
      ...
      await _drag(
        tester,
        [for (var i = 0; i <= 6; i++) Offset(900.0 + i * 20, 800)],
  ...
  testWidgets('"Dibujar con el dedo" hace que el dedo use la herramienta', (
    tester,
  ) async {
    await pumpKraft(tester, size: const Size(1366, 1024));
    await openCanvasEditor(tester);
    ...
    await tester.pumpAndSettle();
    await tester.tap(
      find.byTooltip('Sólo el Apple Pencil dibuja; el dedo selecciona y mueve'),
    );
    await tester.pump();
  ...
  group('borrador', () {
    ...
    test('parcial corta el trazo en dos y se deshace de una vez', () {
      ...
      expect(
        d.strokes.every((s) => s.points.every((p) => (p.dx - 100).abs() > 8)),
        isTrue,
      );
      d.undo();
    ...
    test('por trazo borra el trazo entero y por elemento sólo elementos', () {
      final d = doc()
        ..addStroke(
          _line(CanvasDocument(), const Offset(0, 0), const Offset(200, 0)),
        );
      d
        ..beginErase()
      ...
      expect(
        d.strokes,
        hasLength(1),
        reason: 'el modo elemento no toca trazos',
      );
