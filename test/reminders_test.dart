...
class _FakePlatform implements ReminderPlatform {
  ...
  @override
  Future<void> deleteReminder(String identifier) async =>
      calls.add('apple-delete $identifier');
}
...
void main() {
  ...
  test(
    'programar al crear, reprogramar al cambiar y cancelar al completar o borrar',
    () async {
      ...
      await flush();
      expect(
        platform.scheduled[ReminderService.notificationId(id)],
  ...
  test(
    'con Recordatorios de Apple guarda el identificador y lo borra con la tarea',
    () async {
      expect(await service.setAppleSync(true), isTrue);
      final id = await tasks.create(
        title: 'Comprar lápices',
      ...
      expect(
        platform.calls.where((c) => c.startsWith('apple ')),
        hasLength(1),
        reason: 'guardar el id no vuelve a sincronizar',
      );
  ...
  test('etiqueta legible del recordatorio', () {
    ...
    expect(
      reminderLabel(DateTime(2026, 9, 18, 10, 30), now: now),
      'Viernes, 18 Septiembre · 10:30',
    );
  });
}
