...
const _box = CanvasItem(
  id: 'a',
  type: CanvasItemType.card,
  position: Offset.zero,
);
const _circle = CanvasItem(
  id: 'c',
...
void main() {
  group('geometría', () {
    test(
      'el extremo sale por el lado que mira al otro elemento, no por anclas fijas',
      () {
    ...
    test('la línea se recoloca al mover un elemento', () {
      ...
      final side = linkBetween(
        from: _box,
        fromRect: const Rect.fromLTWH(0, 0, 200, 100),
        to: b,
        toRect: const Rect.fromLTWH(500, 0, 200, 100),
      )!;
      final under = linkBetween(
        from: _box,
        fromRect: const Rect.fromLTWH(0, 0, 200, 100),
        to: b,
        toRect: const Rect.fromLTWH(0, 400, 200, 100),
      )!;
  ...
  group('documento', () {
    ...
    test('conectar no duplica, se deshace y se va con el elemento borrado', () {
      ...
      expect(
        d.addLink('b', 'a'),
        link,
        reason: 'misma pareja en sentido contrario',
      );
      d.addLink('a', 'c');
  ...
  testWidgets('arrastrar el tirador de un elemento hasta otro los conecta', (
    tester,
  ) async {
    ...
    final target = tester.getCenter(
      find.text('Flujo de autenticación sin contraseñas con biometría táctil.'),
    );
  ...
  testWidgets(
    'soltar el tirador en vacío abre la rueda y lo creado queda conectado',
    (tester) async {
      final db = await pumpKraft(tester, size: const Size(1366, 1024));
      await openCanvasEditor(tester);
      ...
      final note = saved.items.firstWhere(
        (i) => i.type == CanvasItemType.sticky && i.id != 'sticky-1',
      );
      expect(
        saved.links.map((l) => (l.from, l.to)),
        contains(('node-2', note.id)),
      );
    },
  );
}
