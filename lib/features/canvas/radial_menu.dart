...
class RadialMenuState {
  const RadialMenuState({required this.center, required this.world, this.hovered, this.held = true});
  ...
  RadialMenuState copyWith({int? hovered, bool clearHovered = false, bool? held}) => RadialMenuState(
        center: center,
        world: world,
...
class RadialMenu extends StatefulWidget {
  const RadialMenu({super.key, required this.state, required this.onSelect, required this.onDismiss});
...
class _RadialMenuState extends State<RadialMenu> with SingleTickerProviderStateMixin {
  late final AnimationController _open = AnimationController(vsync: this, duration: const Duration(milliseconds: 320));
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        if (!s.held) Positioned.fill(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: widget.onDismiss)),
        Positioned(
          left: s.center.dx - r,
          top: s.center.dy - r,
          width: r * 2,
                    ...
                    child: Transform.scale(
                      ...
                      child: Stack(
                        ...
                        children: [
                          Positioned.fill(
                            child: CustomPaint(
                              painter: _RingPainter(count: radialOptions.length, hovered: hovered, progress: t),
                            ),
                          ),
                          ...
                          Center(child: _CenterLabel(label: hovered == null ? 'AÑADIR' : radialOptions[hovered].label.toUpperCase())),
                        ],
                      ),
                    ),
                  ),
...
class _RingPainter extends CustomPainter {
  const _RingPainter({required this.count, required this.hovered, required this.progress});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (isHovered) {
        canvas.drawPath(path.shift(const Offset(3, 3)), Paint()..color = KraftColors.ink);
      }
      canvas.drawPath(path, Paint()..color = isHovered ? KraftColors.primaryContainer : KraftColors.scrim);
      canvas.drawPath(
        path,
        Paint()
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2
          ..color = isHovered ? KraftColors.ink : KraftColors.primaryContainer.withValues(alpha: 0.35),
      );
    }
  }

  @override
  bool shouldRepaint(_RingPainter old) => old.hovered != hovered || old.progress != progress || old.count != count;
}
...
class _SegmentIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final radius = (RadialMenu.outerRadius + RadialMenu.innerRadius) / 2 + (highlighted ? 6 : 0);
    ...
    return Positioned(
      ...
      child: Opacity(
        ...
        child: Transform.scale(
          ...
          child: Icon(icon, size: 22, color: highlighted ? KraftColors.ink : Colors.white),
        ),
      ),
    );
  }
