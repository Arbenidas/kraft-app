...
Future<void> seedCanvas(AppDatabase db) {
  return db.into(db.canvases).insert(CanvasesCompanion.insert(title: 'Boceto de Arquitectura', data: Value(CanvasCodec.seed())));
}
...
Future<void> seedDatabase(AppDatabase db, DateTime now) async {
  ...
  await db.transaction(() async {
    await seedCanvas(db);
    final archId = await db.into(db.projects).insert(ProjectsCompanion.insert(
          title: 'Arquitectura de App v2',
          description: const Value('Diagrama de módulos, sincronización distribuida y persistencia en memoria local.'),
    ...
    await db.into(db.projects).insert(ProjectsCompanion.insert(
          ...
          description: const Value('Flujo de interacción táctil, gestos para Apple Pencil y paletas contextuales.'),
    ...
    await db.into(db.projects).insert(ProjectsCompanion.insert(
          ...
          description: const Value('Ajustes en el motor de trazo suave, curvas de presión y exportación SVG limpia.'),
    ...
    for (final (i, (text, done)) in const [
      ...
      await db.into(db.checklistItems).insert(
            ChecklistItemsCompanion.insert(noteId: noteId, content: text, done: Value(done), position: i),
          );
    }
    await db.into(db.notes).insert(NotesCompanion.insert(
          title: const Value('Reunión de kickoff con cliente'),
          body: const Value('Alcance de la primera entrega, paleta de marca y calendario de revisiones.'),
